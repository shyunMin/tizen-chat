...
```mermaid
graph TD
    subgraph UILayer [UI 레이어 - User Interface Layer]
        direction TB
        PUI[PromptUI]
        GVU[GenUIView]
        CUI[ChatUI]
    ...
    subgraph ServiceLayer [서비스 레이어 - Service Layer]
        direction TB
        CS[ChatService]
        VC[VoiceController]
        SP[STTProvider]
    ...
    subgraph InfraLayer [인프라 레이어 - Infrastructure Layer]
        direction TB
        SE[STTEngine]
        TNB[TizenNativeBridge]
    end
