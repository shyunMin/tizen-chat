...
class ArgotGrpcService {
  ...
  void _handleChatEvent(argot_types.ChatEvent event, {required String turnId}) {
    ...
    if (event.hasToolResult()) {
      ...
      debugPrint('DEBUG: [ArgotGrpc] ToolResult callId=${result.callId} name=${result.name} '
          'err=${result.isError} bytes=${result.outputBytes} truncated=${result.outputTruncated}');
      _eventController.add(
